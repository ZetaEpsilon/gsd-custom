...
tools: Read, Write, Edit, Bash, Grep, Glob, mcp__context7__*, mcp__codex_bridge__*
color: yellow
# hooks:
#   PostToolUse:
#     - matcher: "Write|Edit"
...
1. **If `type="auto"`:**
   - Resolve AI routing hint from either:
     - XML task attributes: `<task ... ai="...">`
     - Markdown task headers: `### Task N [ai="..."]` or `### 14-01-01 [ai="..."]`
   - Apply task_ai_routing using the resolved hint
...
<task_ai_routing>
Task-level AI tags must be honored when possible.

**Accepted tag locations:**
- XML plans: `<task ... ai="codex|opus|opus+codex">`
- Markdown plans: `### Task N [ai="codex|opus|opus+codex"]` or `### <task-id> [ai="codex|opus|opus+codex"]`

**Supported values:**
- `ai="codex"` → implement task via Codex CLI first, then verify in this executor
- `ai="opus+codex"` → use Codex CLI for implementation draft, then refine/finish in this executor
- `ai="opus"` or missing `ai` → execute normally in this executor

**Execution protocol for `ai="codex"` / `ai="opus+codex"` tasks:**
1. Prefer Codex Bridge MCP when available:
   - Use `mcp__codex_bridge__consult_codex` with:
     - `directory`: repo root
     - `format`: `code`
     - `timeout`: `240`
   - Prompt must include: task name, plan path, `<action>`, `<files>`, `<acceptance_criteria>`, `<verify>`, `<done>`, "edit only task-scoped files", and "do not commit".
2. If Codex Bridge MCP is unavailable or fails, resolve a working CLI Codex runner:
```bash
CODEX_RUNNER=""
BASH_CODEX_VERSION="$(codex --version 2>&1)"
BASH_CODEX_STATUS=$?
if echo "$BASH_CODEX_VERSION" | grep -qi "Missing optional dependency @openai/codex-linux-x64"; then
  CODEX_RUNNER="powershell"
elif [ "$BASH_CODEX_STATUS" -eq 0 ]; then
  CODEX_RUNNER="codex"
elif powershell.exe -NoProfile -Command "codex --version" >/dev/null 2>&1; then
  CODEX_RUNNER="powershell"
fi

if [ -n "$CODEX_RUNNER" ]; then
  echo "codex:available:$CODEX_RUNNER"
else
  echo "codex:missing"
fi
```
3. If Bash output includes `Missing optional dependency @openai/codex-linux-x64`, treat that as a shell/runtime mismatch, not "Codex unavailable". Use PowerShell runner for delegation:
```bash
powershell.exe -NoProfile -Command "codex --version"
```
4. Run Codex using the resolved CLI runner:
   - If `CODEX_RUNNER="codex"`: run `codex exec ...` directly.
   - If `CODEX_RUNNER="powershell"`: run `powershell.exe -NoProfile -Command "codex exec ..."` so Windows Codex is used.
5. After Codex returns (via MCP or CLI), run verification in this executor and only then commit using task_commit_protocol.
6. If Codex is missing or delegation fails, continue in this executor and log a deviation:
   - `[Rule 3 - Blocking] codex unavailable for ai=\"codex\" task; executed with Claude executor fallback`

**Safety rules:**
- Never skip task verification just because Codex was used.
- Never let Codex perform final commits for this workflow; commits stay in this executor.
- Treat `ai` tags as task-scoped hints, not plan-level overrides.
</task_ai_routing>
