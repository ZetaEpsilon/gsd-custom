...
Task AI routing (MANDATORY on every `type="auto"` task):
- Use `ai="codex"` for code-heavy implementation work (application logic, components, APIs, tests, refactors).
- Use `ai="opus"` for planning/process/docs/checkpoint tasks and non-code orchestration steps.
- Use `ai="opus+codex"` only when Codex should draft implementation and Opus should finish integration/polish.
- Do not omit `ai` on `type="auto"` tasks.
