...
## Dimension 12: AI Routing Validation

**Question:** Are AI routing attributes valid and sensible?

**Skip if:** No `<task>` elements exist in the plan (non-standard format).

**Checklist:**
- [ ] Every `<task>` element has an `ai` attribute (WARN if missing, don't block — backwards compatible)
- [ ] All `ai` values are one of: `codex`, `opus`, `opus+gemini`, `opus+codex` (BLOCK if invalid value)
- [ ] Tasks with `type="checkpoint:*"` do not have `ai="codex"` (BLOCK — checkpoints need orchestrator context)
- [ ] Tasks marked `ai="codex"` have concrete `<done>` criteria, not vague statements like "works correctly" (WARN)
- [ ] Tasks marked `ai="codex"` touch <=15 files in `<files>` element (WARN if more — may exceed Codex comfort zone)

**Severity:** WARN for missing attrs and soft limits. BLOCK for invalid values and checkpoint violations.
Existing plans without `ai` attributes must pass with warnings (not blocks) for backwards compatibility.

**Example — invalid ai value:**
```yaml
issue:
  dimension: ai_routing
  severity: blocker
  description: "Task 3 has invalid ai value 'gpt'"
  fix_hint: "Use one of: codex, opus, opus+gemini, opus+codex"
```

**Example — codex checkpoint violation:**
```yaml
issue:
  dimension: ai_routing
  severity: blocker
  description: "Task 5 (checkpoint:human-verify) has ai='codex' — checkpoints need orchestrator"
  fix_hint: "Change to ai='opus'"
```

**Example — missing ai attrs (backwards compat):**
```yaml
issue:
  dimension: ai_routing
  severity: warning
  description: "3 of 8 tasks missing ai attribute — will default to opus"
  fix_hint: "Add ai='' to all task elements for explicit routing"
```
