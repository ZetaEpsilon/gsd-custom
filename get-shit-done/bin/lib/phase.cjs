...
function extractTaskAiHints(content) {
  const hints = new Set();
  const taskOpenTagPattern = /<task\b([^>]*)>/gi;
  let taskMatch;
  while ((taskMatch = taskOpenTagPattern.exec(content)) !== null) {
    const attrs = taskMatch[1] || '';
    const aiMatch = attrs.match(/\bai\s*=\s*["']([^"']+)["']/i);
    if (!aiMatch || !aiMatch[1]) continue;
    const value = aiMatch[1].trim().toLowerCase();
    if (value) {
      hints.add(value);
    }
  }
...
function cmdPhasePlanIndex(cwd, phase, raw) {
  ...
  for (const planFile of planFiles) {
    ...
    const aiHints = extractTaskAiHints(content);
    ...
    const plan = {
      ...
      ai_hints: aiHints,
      preferred_ai: aiHints.length === 1 ? aiHints[0] : null,
