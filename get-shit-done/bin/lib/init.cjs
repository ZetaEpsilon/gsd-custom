...
function buildAgentSkillsBlock(config, agentType, projectRoot) {
  ...
  function resolveSkillDir(skillPath) {
    if (!skillPath) return null;
    if (path.isAbsolute(skillPath)) return path.normalize(skillPath);
    if (skillPath.startsWith('~/')) return path.join(homeDir, skillPath.slice(2));
    if (skillPath.startsWith('$HOME/')) return path.join(homeDir, skillPath.slice(6));
    return path.join(projectRoot, skillPath);
  }

  function isUnderAllowedRoots(resolvedPath) {
    const normalized = path.normalize(resolvedPath);
    if (normalized.startsWith(path.normalize(projectRoot + path.sep))) return true;
    return allowedGlobalRoots.some(root => normalized.startsWith(path.normalize(root + path.sep)));
  }

  for (const skillPath of skillPaths) {
    if (typeof skillPath !== 'string') continue;

    const resolvedSkillDir = resolveSkillDir(skillPath);
    if (!resolvedSkillDir) continue;

    // Project-local paths still use strict validator.
    if (!path.isAbsolute(skillPath) && !skillPath.startsWith('~/') && !skillPath.startsWith('$HOME/')) {
    ...
    } else if (!isUnderAllowedRoots(resolvedSkillDir)) {
      process.stderr.write(`[agent-skills] WARNING: Skill path outside allowed roots "${skillPath}" — skipping\n`);
      continue;
    }
    ...
    if (!fs.existsSync(skillMdPath)) {
      process.stderr.write(`[agent-skills] WARNING: Skill not found at "${skillMdPath}" — skipping\n`);
      continue;
    }

    validPaths.push(skillMdPath);
  }

  if (validPaths.length === 0) return '';

  const lines = validPaths.map(p => `- @${p}`).join('\n');
