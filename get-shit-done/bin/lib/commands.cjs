...
/**
 * Parse a Codex JSONL events file and extract the error type.
 * Exit 0 + stdout error type = error found
 * Exit 1 + stdout "no_error" = clean run
 * Exit 2 + stdout "file_not_found" or "parse_error" = input problem
 */
function cmdParseCodexError(eventsPath) {
  if (!eventsPath || !fs.existsSync(eventsPath)) {
    process.stdout.write('file_not_found');
    process.exit(2);
  }
  ...
  // Scan backward — error events may not be the last line
  for (let i = lines.length - 1; i >= 0; i--) {
    try {
      ...
      if (errorType) {
        process.stdout.write(errorType);
        process.exit(0);
      }
    } catch {
...
/**
 * Parse a PLAN.md file and extract task elements with their attributes.
 * Output: JSON array of task objects on stdout. Exit 0 on success, exit 1 if missing/no tasks.
 */
function cmdParsePlanTasks(planPath) {
  if (!planPath || !fs.existsSync(planPath)) {
    process.stderr.write('Plan file not found\n');
    process.exit(1);
  }
  ...
  while ((match = taskBlockRegex.exec(content)) !== null) {
    ...
    tasks.push({
      num,
      name: getChild('name') || getAttr('name') || `task-${num}`,
      type: getAttr('type') || 'auto',
      ai: getAttr('ai') || 'opus',
      files: getChild('files') || getAttr('files') || '',
  ...
  if (tasks.length === 0) {
    process.stderr.write('No tasks found in plan\n');
    process.exit(1);
  }
