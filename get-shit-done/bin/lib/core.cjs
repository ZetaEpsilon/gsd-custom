    ...
    for (const entry of entries) {
      ...
      try {
        ...
        if (now - stat.mtimeMs > maxAgeMs) {
          // Safety: only remove directories when explicitly requested.
          if (stat.isDirectory() && dirsOnly) {
            fs.rmSync(fullPath, { recursive: true, force: true });
          } else if (!dirsOnly && stat.isFile()) {
            // Safety: by default only delete the JSON temp files written by output().
            // Avoid broad prefix-based deletion that could remove unrelated temp files.
            if (prefix === 'gsd-' && !/^gsd-\d+\.json$/.test(entry)) continue;
