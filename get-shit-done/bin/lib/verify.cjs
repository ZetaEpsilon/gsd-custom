...
function cmdVerifyPlanStructure(cwd, filePath, raw) {
  ...
  // Parse and check task elements (XML and markdown task headers)
  const taskPattern = /<task\b([^>]*)>([\s\S]*?)<\/task>/g;
  const markdownTaskPattern = /^###\s+((?:Task\s+\d+|[0-9]+(?:-[0-9]+){2,})[^\n]*)$/gim;
  const tasks = [];
  const aiHints = new Set();
  const allowedTaskAi = new Set(['opus', 'codex', 'opus+codex']);
  let taskMatch;
  while ((taskMatch = taskPattern.exec(content)) !== null) {
    const taskAttrs = taskMatch[1] || '';
    const taskContent = taskMatch[2];
    const typeMatch = taskAttrs.match(/\btype\s*=\s*["']([^"']+)["']/i);
    const taskType = typeMatch ? String(typeMatch[1]).trim().toLowerCase() : null;
    ...
    const aiMatch = taskAttrs.match(/\bai\s*=\s*["']([^"']+)["']/i);
    const taskAi = aiMatch ? String(aiMatch[1]).trim().toLowerCase() : null;
    ...
    if (taskType === 'auto' && !taskAi) {
      errors.push(`Task '${taskName}' missing required ai attribute on type=\"auto\" task`);
    }
    if (taskAi) {
      aiHints.add(taskAi);
      if (!allowedTaskAi.has(taskAi)) {
        errors.push(`Task '${taskName}' has unsupported ai attribute: '${taskAi}'`);
      }
    }

    tasks.push({ name: taskName, type: taskType, ai: taskAi, hasFiles, hasAction, hasVerify, hasDone });
  }

  let markdownTaskMatch;
  while ((markdownTaskMatch = markdownTaskPattern.exec(content)) !== null) {
    const header = markdownTaskMatch[1].trim();
    const taskAiMatch = header.match(/\bai\s*=\s*["']([^"']+)["']/i);
    const taskAi = taskAiMatch ? String(taskAiMatch[1]).trim().toLowerCase() : null;
    const taskName = header.replace(/\s*\[ai\s*=\s*["'][^"']+["']\]\s*$/i, '').trim();

    if (!taskAi) {
      warnings.push(`Markdown task '${taskName}' missing [ai=\"...\"] hint`);
    } else {
      aiHints.add(taskAi);
      if (!allowedTaskAi.has(taskAi)) {
        errors.push(`Markdown task '${taskName}' has unsupported ai attribute: '${taskAi}'`);
      }
    }
  ...
  if (tasks.length === 0) warnings.push('No task definitions found (<task> blocks or markdown task headers)');
  ...
  output({
    ...
    task_ai_hints: Array.from(aiHints),
    tasks,
