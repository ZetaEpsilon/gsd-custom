...
```
Task(
  prompt=filled_debug_subagent_prompt + "\n\n<worktree_branch_check>\nFIRST ACTION: run git merge-base HEAD {EXPECTED_BASE} and capture CURRENT_BRANCH via git symbolic-ref --quiet --short HEAD. If detached, attach to a recovery branch before edits. If merge-base differs from {EXPECTED_BASE}, rebase with REBASE_FROM=$(git merge-base HEAD {EXPECTED_BASE}) and git rebase --onto {EXPECTED_BASE} \"$REBASE_FROM\" HEAD to correct the branch base (fixes Windows EnterWorktree creating branches from main).\n</worktree_branch_check>\n\n<files_to_read>\n- {phase_dir}/{phase_num}-UAT.md\n- .planning/STATE.md\n</files_to_read>\n${AGENT_SKILLS_DEBUGGER}",
