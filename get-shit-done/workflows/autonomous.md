...
`gsd-execute-phase` handles task-level `ai="..."` routing through the executor
for both XML (`<task ... ai="...">`) and markdown plans (`### Task N [ai="..."]`
or `### 14-01-01 [ai="..."]`).
For `ai="codex"` tasks, executor routing is: Codex Bridge MCP first, CLI fallback second
(PowerShell Codex fallback on Windows/Bash mismatch). No extra autonomous-specific
routing step is required.
