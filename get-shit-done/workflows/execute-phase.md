...
Parse JSON for: `phase`, `plans[]` (each with `id`, `wave`, `autonomous`, `objective`, `files_modified`, `task_count`, `ai_hints`, `preferred_ai`, `has_summary`), `waves` (map of wave number → plan IDs), `incomplete`, `has_checkpoints`.
...
3. **Spawn executor agents:**
   ...
   **AI hint semantics:** Execute-phase always dispatches `gsd-executor` in the current
   runtime, and the executor enforces task-level `ai="..."` routing from PLAN.md
   (both `<task ... ai="...">` and markdown forms like `### Task N [ai="..."]`
   or `### 14-01-01 [ai="..."]`).
   For `ai="codex"` (or `ai="opus+codex"`), the executor delegates implementation to
   Codex (prefer Codex Bridge MCP, fallback to CLI), then verifies and commits in the
   executor context. On Windows/Bash mismatch (`@openai/codex-linux-x64`, detected from
   CLI output text even if exit code is misleading), executor must
   route through `powershell.exe ... codex`. If Codex is unavailable, executor falls
   back and logs a deviation.
   ...
   **Sequential dispatch for parallel execution (waves with 2+ agents):**
   ...
   Task(
     ...
     prompt="
       ...
       If `CURRENT_BRANCH` is empty (detached HEAD), attach before doing any work:
       ```bash
       RECOVERY_BRANCH="worktree-recover-$(date +%s)"
       git switch -c "$RECOVERY_BRANCH" || {
         echo "ERROR: Detached HEAD and could not create recovery branch; stop and escalate"
         exit 1
       }
       ...
       ```bash
       REBASE_FROM=$(git merge-base HEAD {EXPECTED_BASE})
       git rebase --onto {EXPECTED_BASE} "$REBASE_FROM" HEAD || {
         echo "WARN: Unable to rebase worktree branch to EXPECTED_BASE; stop and escalate"
         exit 1
       }
       ```
...
5.5. **Worktree cleanup (when `isolation="worktree"` was used):**
   ...
   ```bash
   # List worktrees and iterate safely (supports spaces in paths).
   git worktree list --porcelain | sed -n 's/^worktree //p' | while IFS= read -r WT; do
     ...
     # Only process GSD-managed executor worktrees.
     IS_GSD_WT=false
     if echo "$WT" | grep -Eq '([/\\]|^)\.claude[/\\]worktrees[/\\]agent-'; then
       IS_GSD_WT=true
     fi
     if [ -n "$WT_BRANCH" ] && echo "$WT_BRANCH" | grep -Eq '^worktree-agent-'; then
       IS_GSD_WT=true
     fi
     if [ "$IS_GSD_WT" != "true" ]; then
       echo "INFO: Skipping non-GSD worktree $WT_LABEL at $WT"
       continue
     fi
     ...
     if [ -z "$WT_HEAD" ]; then
       echo "WARN: Could not resolve HEAD for worktree $WT_LABEL at $WT; skipping cleanup"
       continue
     fi

     CURRENT_HEAD=$(git rev-parse HEAD)
     CURRENT_BRANCH_LABEL=$(git rev-parse --abbrev-ref HEAD 2>/dev/null || true)
     WT_BASE=$(git merge-base "$WT_HEAD" "$CURRENT_HEAD" 2>/dev/null || true)
     if [ -z "$WT_BASE" ]; then
       echo "WARN: No merge-base between current branch and worktree $WT_LABEL ($WT); skipping cleanup"
       continue
     fi

     # --- Orchestrator file protection (#1756) ---
     # Snapshot orchestrator-owned files before integration.
     STATE_BACKUP=$(mktemp)
     ...
     # Integrate commits unique to worktree HEAD (covers detached HEAD safely).
     COMMITS=$(git rev-list --reverse "$WT_BASE..$WT_HEAD")
     MERGE_OK=true
     if [ -n "$COMMITS" ]; then
       for C in $COMMITS; do
         git cherry-pick -x "$C" 2>&1 || {
           echo "WARN: Cherry-pick conflict from worktree $WT_LABEL at commit $C; resolve manually"
     ...
     if ! git diff --quiet .planning/STATE.md .planning/ROADMAP.md 2>/dev/null; then
     ...
     # Safety guard: only remove worktree when all worktree patches are integrated.
     UNAPPLIED=$(git cherry HEAD "$WT_HEAD" 2>/dev/null | grep '^[+]' || true)
     if [ -n "$UNAPPLIED" ]; then
       echo "WARN: Some worktree patches are not integrated yet; keeping $WT"
       continue
     fi

     WT_DIRTY=$(git -C "$WT" status --porcelain 2>/dev/null || true)
     if [ -n "$WT_DIRTY" ]; then
       echo "WARN: Worktree $WT_LABEL has uncommitted changes; skipping forced removal to avoid data loss"
       continue
     fi

     git worktree remove "$WT" --force 2>/dev/null || true
     if [ -n "$WT_BRANCH" ] && [ "$WT_BRANCH" != "HEAD" ] && [ "$WT_BRANCH" != "$CURRENT_BRANCH_LABEL" ]; then
