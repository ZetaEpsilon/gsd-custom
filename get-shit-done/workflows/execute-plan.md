...
**Pattern A:** init_agent_tracking → capture `EXPECTED_BASE=$(git rev-parse HEAD)` → spawn Task(subagent_type="gsd-executor", model=executor_model) with prompt: execute plan at [path], autonomous, all tasks + SUMMARY + commit, follow deviation/auth rules, report: plan name, tasks, SUMMARY path, commit hash → track agent_id → wait → update tracking → report. **Include `isolation="worktree"` only if `workflow.use_worktrees` is not `false`** (read via `config-get workflow.use_worktrees`). **When using `isolation="worktree"`, include a `<worktree_branch_check>` block in the prompt** instructing the executor to (1) run `ACTUAL_BASE=$(git merge-base HEAD {EXPECTED_BASE})`, (2) attach to a recovery branch if `git symbolic-ref --quiet --short HEAD` is empty (detached HEAD), and (3) if `ACTUAL_BASE` differs, rebase with `git rebase --onto {EXPECTED_BASE} "$REBASE_FROM" HEAD` (where `REBASE_FROM=$(git merge-base HEAD {EXPECTED_BASE})`). This avoids detached-HEAD commit loss and fixes the Windows `EnterWorktree` wrong-base issue without relying on `git reset --soft`.
...
3. Per task:
   - **MANDATORY read_first gate:** If the task has a `<read_first>` field, you MUST read every listed file BEFORE making any edits. This is not optional. Do not skip files because you "already know" what's in them — read them. The read_first files establish ground truth for the task.
    - `type="auto"`:
      - Resolve task-level AI hint from either `<task ... ai="...">` or markdown task headers like `### Task N [ai="..."]` / `### 14-01-01 [ai="..."]`.
      - `ai="codex"` → route implementation to Codex first, then verify and commit here.
      - `ai="opus+codex"` → draft with Codex first, refine/finish here, then verify and commit.
      - Routing order for Codex tasks:
        - Prefer Codex Bridge MCP (`consult_codex`) when available.
        - Fallback to CLI runner detection (`codex --version`, then `powershell.exe -NoProfile -Command "codex --version"`).
        - Inspect output text, not exit code alone. If output contains `Missing optional dependency @openai/codex-linux-x64`, treat as shell mismatch and use PowerShell Codex.
      - Missing `ai` or `ai="opus"` → execute normally here.
     - If `tdd="true"` → follow TDD execution flow.
     - Implement with deviation rules + auth gates. Verify done criteria. Commit (see task_commit). Track hash for Summary.
...
```bash
node $HOME/.claude/get-shit-done/bin/gsd-tools.cjs commit-to-subrepo "{type}({phase}-{plan}): {description}" --files file1 file2 ...
```
...
**Skip this step if running in parallel worktree mode** (the orchestrator handles STATE.md/ROADMAP.md updates centrally after merging worktrees).
...
**Skip this step if running in parallel worktree mode** (the orchestrator handles ROADMAP.md updates centrally after worktree merges).
...
- STATE.md updated (position, decisions, issues, session) or deferred to orchestrator in worktree mode
- ROADMAP.md updated or deferred to orchestrator in worktree mode
