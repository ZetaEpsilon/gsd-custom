...
**Gemini-never-solo policy check:**
```bash
GEMINI_NEVER_SOLO=$(node "$HOME/.claude/get-shit-done/bin/gsd-tools.cjs" \
  config-get routing.gemini_never_solo 2>/dev/null || echo "true")
```
After CLI detection, before invoking reviewers: if Gemini is the ONLY available
reviewer AND `GEMINI_NEVER_SOLO=true`, display warning:
`"Gemini-never-solo policy active: Gemini is the only available reviewer. Skipping review — Gemini must be paired with at least one other reviewer."`
Exit the review workflow without producing REVIEWS.md.

For each selected CLI, invoke in sequence (not parallel — avoid rate limits):

**Gemini:**
```bash
cat /tmp/gsd-review-prompt-{phase}.md | gemini 2>/dev/null > /tmp/gsd-review-gemini-{phase}.md
```
...
**Codex:**
```bash
cat /tmp/gsd-review-prompt-{phase}.md | codex exec --skip-git-repo-check - > .tmp-codex-review-output.md 2>/dev/null
mv .tmp-codex-review-output.md /tmp/gsd-review-codex-{phase}.md
```
Pipe the review prompt through stdin to `codex exec -`. Save to a temp file first, then move to the expected location. Run from the repo root.
