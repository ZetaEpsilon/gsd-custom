...
```bash
# Guard workspace path before attempting git worktree removal
if [ -z "$WORKSPACE_PATH" ] || [ "$WORKSPACE_PATH" = "/" ] || [ "$WORKSPACE_PATH" = "$HOME" ]; then
  echo "ERROR: Unsafe workspace path: '$WORKSPACE_PATH' — refusing worktree cleanup"
  exit 1
fi

WORKSPACE_REAL=$(cd "$WORKSPACE_PATH" 2>/dev/null && pwd || true)
if [ -z "$WORKSPACE_REAL" ] || [ "$WORKSPACE_REAL" = "/" ] || [ "$WORKSPACE_REAL" = "$HOME" ]; then
  echo "ERROR: Could not resolve safe workspace path from '$WORKSPACE_PATH' — refusing worktree cleanup"
  exit 1
fi

cd "$SOURCE_REPO_PATH"
git worktree remove "$WORKSPACE_REAL/$REPO_NAME" 2>&1 || true
```
...
# Destruction guards
if [ -z "$WORKSPACE_PATH" ] || [ "$WORKSPACE_PATH" = "/" ] || [ "$WORKSPACE_PATH" = "$HOME" ]; then
  echo "ERROR: Unsafe workspace path: '$WORKSPACE_PATH' — refusing to delete"
  exit 1
fi

WORKSPACE_REAL=$(cd "$WORKSPACE_PATH" 2>/dev/null && pwd || true)
if [ -z "$WORKSPACE_REAL" ] || [ "$WORKSPACE_REAL" = "/" ] || [ "$WORKSPACE_REAL" = "$HOME" ]; then
  echo "ERROR: Could not resolve safe workspace path from '$WORKSPACE_PATH' — refusing to delete"
  exit 1
fi
