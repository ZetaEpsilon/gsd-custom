...
If CURRENT_BRANCH is empty (detached HEAD), attach before any edits:
RECOVERY_BRANCH="worktree-recover-$(date +%s)"
git switch -c "$RECOVERY_BRANCH" || { echo "ERROR: detached HEAD recovery failed"; exit 1; }

If ACTUAL_BASE differs from ${EXPECTED_BASE}, run:
REBASE_FROM=$(git merge-base HEAD ${EXPECTED_BASE})
git rebase --onto ${EXPECTED_BASE} "$REBASE_FROM" HEAD || { echo "ERROR: rebase to expected base failed"; exit 1; }
...
1. **Worktree cleanup:** If the executor ran with `isolation="worktree"`, merge the worktree branch back and clean up:
   ...
   git worktree list --porcelain | sed -n 's/^worktree //p' | while IFS= read -r WT; do
     ...
     if [ "$IS_GSD_WT" != "true" ]; then
       echo "INFO: Skipping non-GSD quick worktree $WT_LABEL at $WT"
       continue
     fi

     WT_HEAD=$(git -C "$WT" rev-parse HEAD 2>/dev/null || true)
     if [ -z "$WT_HEAD" ]; then
       echo "WARN: Could not resolve HEAD for quick worktree $WT_LABEL at $WT; skipping cleanup"
       continue
     fi
     ...
     if [ -z "$WT_BASE" ]; then
       echo "WARN: No merge-base between current branch and quick worktree $WT_LABEL ($WT); skipping cleanup"
       continue
     fi
     ...
     if [ -n "$COMMITS" ]; then
       for C in $COMMITS; do
         git cherry-pick -x "$C" 2>&1 || {
           echo "WARN: Cherry-pick conflict from quick worktree $WT_LABEL at commit $C; resolve manually"
    ...
    if [ -n "$UNAPPLIED" ]; then
      echo "WARN: Quick worktree patches are not fully integrated yet; keeping $WT"
      continue
    fi

    WT_DIRTY=$(git -C "$WT" status --porcelain 2>/dev/null || true)
    if [ -n "$WT_DIRTY" ]; then
      echo "WARN: Quick worktree $WT_LABEL has uncommitted changes; skipping forced removal to avoid data loss"
      continue
    fi
