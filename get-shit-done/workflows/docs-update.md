...
**Step 3.5: Gemini Documentation Research (optional)**

```bash
GEMINI_ENABLED=$(node "$HOME/.claude/get-shit-done/bin/gsd-tools.cjs" \
  config-get routing.gemini_enabled 2>/dev/null || echo "true")
command -v gemini >/dev/null 2>&1 && GEMINI_OK=true || GEMINI_OK=false
```

If `GEMINI_OK=true` AND `GEMINI_ENABLED=true`:

1. Build a single prompt listing all docs in the manifest with their types and targets
2. Ask Gemini to research the codebase and provide factual context for each doc
3. Dispatch via pipe transport (single invocation, not per-doc):
   ```bash
   cat /tmp/gsd-gemini-docs-research.md | gemini \
     > .artifacts/docs-update-task-00-gemini-result.txt 2>/dev/null
   GEMINI_EXIT=$?
   ```
4. If `GEMINI_EXIT=0`: read the output file and pass as a `<gemini_context>` block
   in each doc-writer agent prompt:
   ```xml
   <gemini_context>
   [Contents of Gemini's documentation research]

   NOTE: Use as additional context for factual accuracy. Validate claims
   against the codebase before including them in documentation.
   </gemini_context>
   ```

If unavailable or fails: skip silently — doc-writers proceed without Gemini context.
This is lighter than research-phase because docs-update dispatches parallel
doc-writer agents. One Gemini pre-pass feeds all writers.
