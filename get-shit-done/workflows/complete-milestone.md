...
if [ "$BRANCHING_STRATEGY" = "phase" ]; then
  for branch in $PHASE_BRANCHES; do
    if [ -z "$branch" ] || [ "$branch" = "$CURRENT_BRANCH" ]; then
      echo "WARN: Skipping unsafe branch delete target '$branch'"
      continue
    fi
...
if [ "$BRANCHING_STRATEGY" = "milestone" ]; then
  if [ -n "$MILESTONE_BRANCH" ] && [ "$MILESTONE_BRANCH" != "$CURRENT_BRANCH" ]; then
    git branch -d "$MILESTONE_BRANCH" 2>/dev/null || git branch -D "$MILESTONE_BRANCH"
  else
    echo "WARN: Skipping unsafe milestone branch delete target '$MILESTONE_BRANCH'"
  fi
fi
```
